  ...
  void _parseMockSignals() {
    ...
    for (var signal in _signals) {
      _priceControllers.add(
        TextEditingController(text: signal.price?.toInt().toString() ?? '0'),
      );
    ...
    for (var signal in _signals) {
      if (signal.confidence < 0.75) {
        _showWarning = true;
        _warningMessage =
            'Sinyal dari "${signal.brand}" memiliki tingkat kepercayaan rendah. Silakan tinjau kembali data di bawah.';
      }
    }
  }
  ...
  void _validateAndSubmit() {
    ...
    for (int i = 0; i < _signals.length; i++) {
      ...
      if (priceVal < 500 || priceVal > 15000) {
        ...
        errMsg =
            'Harga untuk ${_signals[i].brand} (Rp ${priceVal.toInt()}) mencurigakan (terlalu rendah/tinggi). Silakan verifikasi.';
        break;
      }
    }
    ...
    final aiAnalysis = AIAnalysis(
      summary:
          'Analisis dari ${editedSignals.length} sinyal kompetitor di ${widget.outlet.name}',
      keyFindings: editedSignals
          .map((s) => '${s.brand} ${s.product}: ${s.signalType}')
          .toList(),
      recommendations: editedSignals
          .map((s) => 'Pantau ${s.brand} ${s.product}')
          .toList(),
    ...
    final report = appState.addReport(
      ...
      content: widget.text,
      inputType: widget.inputMode == 'voice'
          ? ReportInputType.voice
      ...
      body: Column(
        children: [
          TacoPremiumHeader(
            ...
            subtitle: 'Tinjau dan sesuaikan data ekstraksi',
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SectionHeader(
                    ...
                    subtitle:
                        'Tinjau dan sesuaikan parameter sinyal pasar berikut',
                  ),
                  const SizedBox(height: 12),
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              TacoBadge(
                                text:
                                    'Keyakinan: ${(signal.confidence * 100).toInt()}%',
                                backgroundColor: signal.confidence >= 0.8
                                    ? AppColors.successLight
                                    : AppColors.warningLight,
                                textColor: signal.confidence >= 0.8
                                    ? AppColors.success
                                    : AppColors.warning,
                              ),
                            ],
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: const [
                              DropdownMenuItem(
                                value: '20',
                                child: Text('Sangat Rendah (20%)'),
                              ),
                              DropdownMenuItem(
                                value: '40',
                                child: Text('Rendah (40%)'),
                              ),
                              DropdownMenuItem(
                                value: '60',
                                child: Text('Cukup (60%)'),
                              ...
                              DropdownMenuItem(
                                value: '80',
                                child: Text('Banyak (80%)'),
                              ),
                              DropdownMenuItem(
                                value: '100',
                                child: Text('Penuh (100%)'),
                              ),
                            ],
                        ...
                        child: TacoButton(
                          ...
                          onPressed: () {
                            Navigator.popUntil(
                              context,
                              (route) =>
                                  route.settings.name == '/report/input-mode',
                            );
                          },
                        ),
                      ),
